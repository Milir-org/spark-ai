...
import { campaignsTable, blueprintsTable, providerDraftsTable, approvalsTable } from "@workspace/db";
...
router.post("/campaigns", async (req, res) => {
  ...
  // Seed mock provider drafts for every new campaign
...
router.get("/campaigns/:id", async (req, res) => {
  ...
  return res.json({
    ...
    blueprint: blueprint ? formatBlueprint(blueprint) : null,
...
router.post("/campaigns/:id/blueprint", async (req, res) => {
  ...
  const negativeKeywordThemes: NegativeThemeEntry[] = [
    { id: 1, name: "Free / DIY Seekers", rationale: "High volume but low commercial intent — will inflate impressions and drive up CPC.", terms: ["free", "diy", "how to", "tutorial", "course"] },
    { id: 2, name: "Job Seekers", rationale: "Common contamination in SaaS searches — no conversion value.", terms: ["jobs", "career", "salary", "hiring", "internship"] },
    { id: 3, name: "Irrelevant Verticals", rationale: "Adjacent but non-relevant search terms from unrelated industries.", terms: ["real estate", "restaurant", "ecommerce store", "shopify"] },
  ];
  ...
  const adDirection: AdDirectionEntry = {
    ...
    headlines: [
      "AI Campaigns That Actually Deliver",
      `Cut CPL by 40% — Powered by SPARK AI`,
  ...
  const [blueprint] = await db.insert(blueprintsTable).values({
    ...
    strategySummary: `AI-designed ${goal} campaign for "${c.name}". Multi-channel approach leveraging paid search and retargeting to maximise qualified reach across ${audience} in ${geo}, with smart budget allocation based on predicted channel performance.`,
    audienceStrategy: `Primary: ${audience}. Segmentation: Behavioural + firmographic. Top segments: High-intent website visitors (74 contacts), dormant CRM leads >90 days (312 contacts), LinkedIn-matched audience (2,400 prospects). Exclude: current customers, bounced contacts.`,
    budgetPlan: `Total: $${budget.toLocaleString()}. Split: Google Ads 65% ($${(budget * 0.65).toFixed(0)}), Microsoft Advertising 25% ($${(budget * 0.25).toFixed(0)}), Retargeting 10% ($${(budget * 0.10).toFixed(0)}). Pacing: ${spendStyle} — ramp weeks 1–2, peak weeks 3–4.`,
    ...
    creativePlan: `Asset pack: 3 ad copy variants (A/B/C), 2 landing page headline variants, 5 responsive search ads per ad group. Brand tone: professional, confident, outcome-focused.`,
    experimentPlan: `A/B Test 1: Landing headline — "Grow faster" vs "Generate more qualified leads". A/B Test 2: CTA — "Book a Demo" vs "Start Free Trial". Duration: 14 days per test. Success: CPA reduction ≥15%.`,
    measurementPlan: `Primary KPIs: Cost per qualified lead (target <$45), Lead-to-opportunity rate (>22%), ROAS (>3.5×). Secondary: CTR >2.5%, conversion rate >3.8%. Daily alerts, weekly AI report.`,
    trackingPlan: `GA4 conversion events: form_submit (primary), demo_booked (primary), page_view_pricing (secondary). Google Tag Manager container required. Offline conversion import via CRM webhook for closed-won tracking.`,
    platformStrategy,
    keywordThemes,
    ...
    executionChecklist: [
      ...
      "Enable automated bidding after 50+ conversions accumulated",
      "Schedule weekly SPARK AI performance review",
      "Set budget pacing alerts at 70% and 90% thresholds",
    ],
    ...
    risks: [
      ...
      "Competitor conquest terms carry higher CPCs and moderate conversion rates — monitor CPL weekly",
      "Microsoft Advertising account may require additional verification time",
    ],
    assumptions: [
      "Landing page conversion rate baseline: 3.8%",
      "Target geography has sufficient search volume for all keyword themes",
      "Creative assets are available within 5 business days of brief",
    ],
  }).returning();

  // Update provider draft readiness scores now blueprint exists
  await updateProviderReadiness(id, blueprint ? 85 : 50, blueprint ? 72 : 40);
...
router.get("/campaigns/:id/approval-blockers", async (req, res) => {
  ...
  const blockers = [];
  if (!hasBlueprint) blockers.push({ type: "missing_blueprint", label: "Blueprint not generated", severity: "error" });

  const drafts = await db.select().from(providerDraftsTable).where(eq(providerDraftsTable.campaignId, id));
  for (const draft of drafts) {
    const issues = (draft.validationIssues as ProviderValidationIssue[] ?? []).filter((i) => i.severity === "error");
    if (issues.length) {
      blockers.push({ type: "provider_validation", label: `${draft.provider}: ${issues.length} validation error(s)`, severity: "error" });
    }
  }
...
async function seedProviderDrafts(campaignId: number, scores: { google: number; bing: number }) {
  const googleIssues: ProviderValidationIssue[] = scores.google >= 80 ? [] : [
    { field: "conversionTracking", severity: "warning", message: "Conversion tracking not verified in Google Ads account" },
  ];
  const bingIssues: ProviderValidationIssue[] = scores.bing >= 70 ? [] : [
    { field: "accountVerification", severity: "warning", message: "Microsoft Advertising account requires additional verification" },
  ];
  await db.insert(providerDraftsTable).values([
    {
      campaignId,
      provider: "google_ads",
      readinessScore: scores.google,
      status: scores.google > 0 ? "draft_ready" : "not_started",
      validationIssues: googleIssues,
      draftSummary: "Campaign structure imported. Keywords and ads pending final review.",
      accountStatus: "active",
      syncStatus: scores.google > 0 ? "synced" : "not_synced",
    },
    {
      campaignId,
      provider: "microsoft_advertising",
      readinessScore: scores.bing,
      status: scores.bing > 0 ? "draft_ready" : "not_started",
      validationIssues: bingIssues,
      draftSummary: "Imported from Google Ads structure. Bid adjustments pending.",
      accountStatus: "active",
      syncStatus: scores.bing > 0 ? "synced" : "not_synced",
    },
  ]);
}
...
async function updateProviderReadiness(campaignId: number, googleScore: number, bingScore: number) {
  ...
  for (const draft of drafts) {
    ...
    const issues: ProviderValidationIssue[] = score >= 80 ? [] : [
      { field: "conversionTracking", severity: "warning", message: "Verify conversion tracking before launch" },
    ];
    await db.update(providerDraftsTable).set({
      readinessScore: score,
      status: "draft_ready",
      validationIssues: issues,
      syncStatus: "synced",
    }).where(eq(providerDraftsTable.id, draft.id));
  }
}
...
function formatBlueprint(b: any) {
  return {
    ...
    strategicAngle: b.strategicAngle ?? null,
    strategySummary: b.strategySummary,
    audienceStrategy: b.audienceStrategy,
    budgetPlan: b.budgetPlan,
    channelPlan: b.channelPlan,
    creativePlan: b.creativePlan,
    experimentPlan: b.experimentPlan,
    measurementPlan: b.measurementPlan,
    trackingPlan: b.trackingPlan ?? null,
    platformStrategy: b.platformStrategy ?? [],
    keywordThemes: b.keywordThemes ?? [],
    negativeKeywordThemes: b.negativeKeywordThemes ?? [],
    adDirection: b.adDirection ?? null,
